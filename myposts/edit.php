                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <form method="POST" enctype="multipart/form-data">
                                <input name="title" value="<?php echo $old_post_data['title']; ?>" type="text"
                                    placeholder="title" class="form-control" />
                                <?= isset($errors['title']) ? "<span class='text-danger'>" . $errors['title'] . "</span>" : "" ?>
                                <textarea name=" content" value="<?php echo $old_post_data['content']; ?>"
                                    placeholder="content" class="form-control"></textarea>
                                <?= isset($errors['content']) ? "<span class='text-danger'>" . $errors['content'] . "</span>" : "" ?>
                                <label>Upload Image<input type="file"
                                        value="<?php echo BASE_URL.$post['image'].$old_post_data['image']; ?>"
                                        name="image" /></label><br />
                                <?= isset($errors['image']) ? "<span class='text-danger'>" . $errors['image'] . "</span>" : "" ?>
                                <label>Publish date<input type="date"
                                        value="<?php echo date('Y-m-d',strtotime($old_post_data['publish_date'])); ?>"
                                        name="publish_date" class="form-control"></label>
                                ...
                                <select name="tags[]" multiple class="form-control"
                                    value="<?php echo $old_post_data['tags']; ?>">
                                    <?php
                                    foreach ($tags as $tag)
                                    {
                                        if (count($tags) > 0)
                                        {
                                            echo "<option " . (($tags['id']== $old_post_data['tags']['id']) ? "selected" : "") . " value='{$tag['id']}'>{$tag['name']}</option>";
                                        }
                                        else
                                        {
